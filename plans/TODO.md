...
- [x] **ARB-160** — Web manifest (name, icon, theme colour, standalone) generated by vite-plugin-pwa. (§8.6)
- [x] **ARB-161** — `vite-plugin-pwa` configured (Workbox); SW registers in production only. (§8.6)
- [x] **ARB-162** — Map tile caching (cache-first, 30-day TTL) in the SW runtime cache. (§8.6)
- [x] **ARB-163** — API GET caching (network-first, incl. photo bytes) in the SW + app-level `cachedRead`. (§8.6)
- [x] **ARB-164** — Offline store: `lib/offline-cache.ts` (lightweight IndexedDB key→value). Full Dexie schema deferred to the v1.1 write queue. (§8.6)
- [x] **ARB-165** — `cachedRead` writes each successful GET into the cache. (§6.10)
- [x] **ARB-166** — Offline read: `cachedRead` falls back to cache on network failure (not on server errors). (§6.10)
- [x] **ARB-167** — Offline indicator (`OfflineBanner`) in the shell. _Per-screen last-sync timestamp: follow-up._ (§6.10)
- [ ] **ARB-168** — Custom "Add to home screen" prompt. _Installable now via the browser's native prompt; a custom `beforeinstallprompt` UI + iOS instructions are a follow-up._ (§6.2)
